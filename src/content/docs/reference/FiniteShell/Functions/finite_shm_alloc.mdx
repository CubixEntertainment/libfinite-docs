...
The `finite_shm_alloc` function allocates a new shared memory buffer to allow rendering to a [`FiniteShell`](../../finiteshell).
...
When creating an overlay Developers should not ateempt to set the shared memory buffer (shm) until **AFTER** calling [`finite_overlay_set_size_and_position`](../finite_overlay_set_size_and_position)
