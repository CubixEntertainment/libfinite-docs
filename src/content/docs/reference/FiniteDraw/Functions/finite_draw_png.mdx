...
<Aside> finite_draw_png was last modifed in **v0.8.0** </Aside>

The `finite_draw_png` function attempt to draw a png image to screen at a given size and position.

```c
void finite_draw_png(FiniteShell *shell, const char *path, double x, double y, double width, double height, cairo_surface_t **cache,  FiniteColorGroup *fillOnFail);
```
...
| `cairo_surface_t **cache`      | Optional `cairo_surface_t` to cache the image to for faster redraws           |
...
finite_draw_png(myShell, fullPath, x, y, image_width, image_height, NULL, &fs);
```
